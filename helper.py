...
def athletes_over_time(df, col):
    data_over_time = df.drop_duplicates(["Year", col])['Year'].value_counts().reset_index().sort_values('Year')
    data_over_time.rename(columns={'Year': 'Edition', 'count': "No. of athletes"}, inplace=True)
    return data_over_time
...
def most_successful(df, sport):
    ...
    temp_df = temp_df['Name'].value_counts().head(15).reset_index().merge(df, on = 'Name', how = 'left')[['Name', 'count', 'Sport', 'region']].drop_duplicates('Name')
...
def countrywise_most_successful(df, country):
    ...
    temp_df = temp_df['Name'].value_counts().head(10).reset_index().merge(df, on = 'Name', how = 'left')[['Name', 'count', 'Sport', 'region']].drop_duplicates('Name')
    temp_df.rename(columns = {'count' : 'Medals'}, inplace = True)
    return temp_df.reset_index().drop(['index'], axis=1)


def weight_v_height(df, sport):
    athlete_df = df.drop_duplicates(subset = ['ID'])
